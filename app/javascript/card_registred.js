const pay = () => {
  ...
  form.addEventListener("submit", (e) => { // イベント発火
    ...
    // カード情報の取得先
  const formResult = document.getElementById("registre-form");
  ...
  const card = { // カードオブジェクトを生成
    ...
    Payjp.createToken(card, (status, response) => {
      if (status === 200) {
        ...
        const renderDom = document.getElementById("registre-form");   //idを元に要素を取得
        ...
        document.getElementById("registre-form").submit();
        document.getElementById("registre-form").reset();
      }   
    });
  });
 };
